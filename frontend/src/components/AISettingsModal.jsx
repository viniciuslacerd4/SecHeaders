...
export default function AISettingsModal({ isOpen, onClose }) {
  ...
  const loadModelsForSaved = useCallback(
    async (provId) => {
  ...
  async function handleSaveKey() {
    ...
    try {
      ...
      const defaultModel =
        PROVIDERS.find((p) => p.id === provId)?.defaultModel || "";
      const modelToSave = modelsList.includes(defaultModel)
        ? defaultModel
        : modelsList[0] || defaultModel;
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            {/* Header gradient */}
            <div className="absolute top-0 inset-x-0 h-px bg-linear-to-r from-transparent via-primary-400/50 to-transparent" />
            ...
            <div className="px-6 pb-6 space-y-4 overflow-y-auto flex-1">
              ...
              {activeProvider && storedKeys[activeProvider] ? (
                <motion.div
                  ...
                  className={`flex items-start gap-3 p-3.5 rounded-xl ${
                    PROVIDERS.find((p) => p.id === activeProvider)?.bg ||
                    "bg-primary-500/10"
                  } border ${
                    PROVIDERS.find((p) => p.id === activeProvider)?.border ||
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-surface-400 mt-0.5">
                      ...
                      <span className="text-surface-300 font-mono">
                        {storedKeys[activeProvider]?.model || "não definido"}
                      </span>
                    </p>
                    {defaultLLM?.available && (
                      <p className="text-xs text-surface-500 mt-1">
                        A IA padrão do servidor ({defaultLLM.provider}/
                        {defaultLLM.model}) não será utilizada.
                      </p>
                    )}
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-surface-400 mt-0.5">
                      O SecHeaders já possui uma IA configurada (
                    ...
                    <p className="text-xs text-surface-500 mt-1.5">
                      Se preferir, adicione sua própria API key do OpenRouter
                      abaixo para usar outro modelo.
                    </p>
                  </div>
                </motion.div>
              ) : null}
              ...
              <div className="flex items-center gap-2 p-2.5 bg-surface-800/40 border border-surface-700/30 rounded-lg">
                ...
                <p className="text-[11px] text-surface-500">
                  API keys são criptografadas no servidor (AES-256). Nunca são
                  expostas após o salvamento.
                </p>
              </div>
                ...
                {PROVIDERS.map((prov) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span
                          className={`w-2 h-2 rounded-full ${isSaved ? prov.dot : "bg-surface-600"}`}
                        />
                        <span
                          className={`text-sm font-medium flex-1 ${prov.color}`}
                        >
                          {prov.name}
                        </span>
                                  ...
                                  <div>
                                    ...
                                    >
                                      <button
                                        ...
                                        className={`w-full flex items-center justify-between px-3 py-2 bg-surface-950/50 border border-surface-700/40 rounded-lg text-xs transition-colors ${
                                          models.length > 0
                                            ? "cursor-pointer hover:border-surface-600"
                                    ...
                                    {models.length > 0 && (
                                      <p className="mt-1 text-[11px] text-surface-500">
                                        {models.length} modelo
                                        {models.length !== 1 ? "s" : ""}{" "}
                                        disponíve
                                        {models.length !== 1 ? "is" : "l"}
                                      </p>
                                    )}
                              ...
                              {(!isSaved || isEditing) && (
                                <>
                                  <div>
                                    ...
                                    />
                                    <p className="mt-1 text-[11px] text-surface-500">
                                      A key será criptografada e nunca mais
