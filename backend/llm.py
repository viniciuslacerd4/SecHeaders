...
_PROMPT_VERSION = "v3"  # Incrementar ao mudar o prompt para invalidar o cache

def _cache_key(header_name: str, issues: list[str], severity: str) -> str:
    """Gera uma chave de cache baseada nos parâmetros e versão do prompt."""
    raw = f"{_PROMPT_VERSION}|{header_name}|{'|'.join(sorted(issues))}|{severity}"
...
REGRA CRÍTICA: Você DEVE retornar EXATAMENTE 5 seções, nem mais nem menos. \
Use EXATAMENTE os títulos abaixo precedidos de ##. \
NÃO omita nenhuma seção. NÃO adicione seções extras. NÃO mude os títulos.
...
## Risco real
Descreva detalhadamente os riscos REAIS e CONCRETOS para o site e seus usuários. Explique o impacto para usuários finais, empresa e dados.

## Exemplos de ataque
Forneça exemplos PRÁTICOS de como um atacante poderia explorar essa falha, com comandos ou payloads demonstrativos em blocos de código (bash, python, html ou javascript). Esses exemplos são para uso em AMBIENTE CONTROLADO pelo blue team.

## Como corrigir
Forneça a correção EXATA com exemplos de configuração para Nginx, Apache e Node.js/Express em blocos de código.

## Teste de validação
Forneça comandos curl ou scripts para validar a correção antes e depois, em blocos de código bash.

LEMBRETE FINAL: A resposta deve conter EXATAMENTE estas 5 seções na ordem acima. Nenhuma a mais, nenhuma a menos."""
...
NÃO adicione rodapés, assinaturas, dados de contato fictícios, frases de encerramento como "Relatório gerado por..." ou qualquer texto fora das 5 seções acima. Termine o relatório exatamente após o último item do Checklist de validação Blue Team."""
...
async def _call_llm(prompt: str, max_tokens: int = 3000) -> str:
    ...
    client = AsyncOpenAI(api_key=LLM_API_KEY, base_url=OPENROUTER_BASE_URL)
    response = await client.chat.completions.create(
        model=LLM_MODEL,
        messages=[
            {
                "role": "system",
                "content": "Você é um especialista sênior em segurança web, pentesting e cybersecurity com mais de 15 anos de experiência. Você gera relatórios detalhados, didáticos e práticos para times de segurança (Blue Team). Sempre use blocos de código com a linguagem especificada para exemplos técnicos.",
            },
...
def has_default_llm() -> bool:
    """Verifica se existe uma API key do OpenRouter configurada."""
...
async def explain_header(
    ...
    encontrados em um header específico. Usa cache para evitar chamadas duplicadas.
    """
...
async def generate_summary(
    ...
    for key, data in headers_data.items():
        status = (
            "✅ OK"
            if not data.get("issues") or data.get("severity") == "info"
            else f"⚠️ {data['severity'].upper()}"
        )
        present = "Presente" if data.get("present") else "Ausente"
        value_info = (
            f" | Valor: {data.get('value', 'N/A')[:100]}" if data.get("present") else ""
        )
...
async def explain_all_headers(
    headers_data: dict, llm_config: dict | None = None
) -> dict[str, str]:
    """
...
def _ok_explanation(header_name: str, header_value: str | None = None) -> str:
    """Gera feedback positivo para headers que estão OK — sempre 3 seções fixas."""
    ok_data = _OK_DESCRIPTIONS.get(header_name)
    fallback_data = _FALLBACK_DESCRIPTIONS.get(header_name)

    value_info = f"\n\nValor configurado: `{header_value}`" if header_value else ""

    if ok_data and fallback_data:
        return f"""## Tudo certo
✅ {ok_data["good"]}{value_info}
...
## O que é este header
O **{header_name}** é um mecanismo de segurança HTTP que contribui para a proteção do site e de seus usuários.

## Risco real
A ausência ou má configuração deste header poderia expor o site a vulnerabilidades de segurança. Mantenha-o sempre ativo e atualizado."""
...
def _fallback_summary(
    url: str, score: float, classification: str, headers_data: dict
) -> str:
...
curl -s -I https://"""
        ...
        + """ | grep -iE 'strict-transport|content-security|x-frame|x-content-type|referrer-policy|permissions-policy|set-cookie'
```"""
    )
